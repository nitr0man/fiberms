...
<input type="hidden" value="-1" name="curr_fiber">
	<table id="contable">
		...
		<tr>
		<td><label class="events_anonce">Точка</label></td><td> <select name="CableLinePoint" onChange="javascript: GetFiber(document.fibersplice.CableLinePoint.value,document.fibersplice.NetworkNodeId.value,document.fibersplice.curr_fiber.value,3);"> {html_options values=$ComboBox_CableLinePoint_values selected=$ComboBox_CableLinePoint_selected output=$ComboBox_CableLinePoint_text}</select></td>
		</tr>
		<tr>
