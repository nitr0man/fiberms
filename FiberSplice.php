...
if ($_SERVER["REQUEST_METHOD"] == 'POST'){
	...
	if ($_POST['mode'] == 1) {
		...
		$res = FiberSplice_Mod( $SpliceId, $cableLine, $fiber, $FiberSpliceOrganizer );
		if (isset($res['error'])) {
           	$message = $res['error'];
			$error = 1;
        } elseif ($res == 1) {
		...
		$res = FiberSplice_Add( $CableLine1, $fiber1, $CableLine2, $fiber2, $FiberSpliceOrganizer, $NetworkNodeId );
