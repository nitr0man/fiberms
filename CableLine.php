...
else
{
    ...
    {        
        ...
        else
        {
            ...
            $res = getCableLineList( $sort, $wr, $config[ 'LinesPerPage' ],
                    ($page - 1) * $config[ 'LinesPerPage' ] );
            if ( $res[ 'count' ] < 1 )
            {
    ...
    elseif ( ($_GET[ 'mode' ] == 'charac') and (isset( $_GET[ 'cablelineid' ] )) )
    {
        ...
        $smarty->assign( "CableType",
                '<a href="CableType.php?mode=charac&cabletypeid='.$rows[ 0 ][ 'CableTypeId' ].'">'.$rows[ 0 ][ 'CableTypeMarking' ].'</a>' );
